...
This component can optionally display errors if passed in a termWarning or a prerequisiteWarning. The card will
have a default background color, but have either a yellow or red border respectively to indicate the errors.
...
5. `termWarning`: An error related to the semester the card is placed in

```javascript
// A boolean
```

6. `prerequisiteWarning`: The card's prerequisites are not met
